...
// Pin mit Relais
const int PinHeatingRelais = 8; // Nummer noch nicht richtig
float HeatingTempSchwellwert = 10; // Temperatur bei deren Unterschreitung die Heizung nachts aktiviert wird
...
void setup()
{
  ...
  pinMode(PinHeatingRelais,OUTPUT);
...
void HeizungSteuerung(){
  if( (Sollposition == UNTEN) && (TempMittel < HeatingTempSchwellwert) ){ // Nur einschalten wenn Klappe geschlossen ist und Temperatur Schwellwert unterschreitet
    digitalWrite(PinHeatingRelais,HIGH);
  }
  else{
    digitalWrite(PinHeatingRelais,LOW);
  }
}
